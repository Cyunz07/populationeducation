...
async function loadAll() {
  ...
  try {
    const s = localStorage.getItem('g10-students-v2');
    if (s) students = JSON.parse(s);
  } catch (e) {}
  try {
    const e = localStorage.getItem('g10-entries-v2');
    if (e) entries = JSON.parse(e);
  } catch (e) {}
  try {
    const m = localStorage.getItem('g10-marks-v1');
    if (m) marks = JSON.parse(m);
  } catch (e) {}
  if (!students) {
...
async function saveStudents(students) {
  try { localStorage.setItem('g10-students-v2', JSON.stringify(students)); } catch (e) {}
}
async function saveEntries(entries) {
  try { localStorage.setItem('g10-entries-v2', JSON.stringify(entries)); } catch (e) {}
}
async function saveMarks(marks) {
  try { localStorage.setItem('g10-marks-v1', JSON.stringify(marks)); } catch (e) {}
}
...
export default function App() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: COLORS.cream, fontFamily: 'Inter, sans-serif', borderRadius: 8, overflow: 'hidden', border: `1px solid ${COLORS.greenSoft}` }}>
      <style>{FONT_IMPORT}</style>
